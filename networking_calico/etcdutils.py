        ...
        while not self._stopped:
            ...
            try:
                ...
                if cluster_id != current_cluster_id:
                    # No particular handling here, because we do a complete
                    # reread every WATCH_TIMEOUT_SECS anyway; but keep track of
                    # the current cluster ID and log if it changes.
            ...
            def _cancel_watch_if_broken():
                ...
                while not self._stopped:
                    ...
                    # Write to a key in the tree that we are watching.  If the
                    # watch is working normally, it will report this event.
                    #
                    # The suffix "////" is chosen to eliminate the possibility
                    # of libcalico-go handling this as a valid resource, and to
                    # minimize the chance of that happening in future as
                    # libcalico-go is enhanced.  Current libcalico-go regards a
                    # resource name as invalid if it contains any slash
                    # characters.
                    etcdv3.put(self.prefix + "////", str(time_now))
