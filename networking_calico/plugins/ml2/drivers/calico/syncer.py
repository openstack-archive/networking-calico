...
class ResourceSyncer(object):
    ...
    Profile, NetworkPolicy and Subnet resources that this driver writes into
    etcd, as the Calico equivalent of the subset of the Neutron data model that
    we support.  WorkloadEndpoint, Profile and NetworkPolicy resources are
    written according to the v3 Calico data model; necessarily so because that
    is the data format that Felix now expects.  Subnet resources are written
    according to an adhoc v1 format that is essentially private within
    networking-calico; which is OK because they are only read by the Calico
    DHCP agent.

    Calico resource types are not completely 1:1 with Neutron resource types,
    so it's important to note that this class is instanced per _Calico_
    resource type.  Specifically, the instances of this class for Profile and
    NetworkPolicy resources both generate the Calico data that they write from
    Neutron security groups.  On the other hand, there is a 1:1 mapping for
    WorkloadEndpoints (from Neutron ports) and for Subnets (from Neutron
    subnets).
    ...
    - submits an etcd transaction to write corresponding Calico data only if
      that creates the relevant etcd key
    ...
    does so if the mod_revision of the relevant etcd key is still what it was
    ...
    def resync_start(self, context):
        ...
        # The set of etcd resources that should be deleted at the end of this
        # resync - as a map from resource name to mod_revision.
        # resync_finish() will delete these.
        ...
        for etcd_resource in etcd_resources:
            ...
            if name in neutron_map:
                ...
                LOG.warning("etcd deletion needed for %s %s",
                            self.resource_kind, name)
                self.names_to_delete[name] = mod_revision
    ...
    def resync_finish(self):
        for name, mod_revision in self.names_to_delete.iteritems():
