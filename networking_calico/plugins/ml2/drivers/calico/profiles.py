...
class ProfileSyncer(V3ResourceSyncer):

    def __init__(self, db, txn_from_context):
        super(ProfileSyncer, self).__init__(db,
                                            txn_from_context,
                                            "Profile")

    # Specialize the base implementation of this method so as to filter out any
    # Profiles whose name does not being with our prefix.
    def get_all_from_etcd(self):
        results = []
        for t in super(ProfileSyncer, self).get_all_from_etcd():
            name, spec, mod_revision = t
            if name.startswith(OPENSTACK_SG_PREFIX):
                results.append(t)
        return results

    def get_all_from_neutron(self, context):
        return dict((with_openstack_sg_prefix(sg['id']), sg)
    ...
    def neutron_to_etcd_write_data(self, sg, context, reread=False):
        ...
        return profile_spec(sg['id'], rules)
    ...
    def write_sgs_to_etcd(self, sgids, context):
        ...
        for sgid in sgids:
            self.update_in_etcd(with_openstack_sg_prefix(sgid),
                                profile_spec(sgid, rules))


def profile_spec(sgid, rules):
    """Generate JSON ProfileSpec for the given security group."""

    # rules can include those for several security groups.  Pick out the rules
    # for the security group that we are translating right now.
