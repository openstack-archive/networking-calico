...
class TestStatusWatcher(_TestEtcdBase):
    ...
    def test_snapshot(self):
        ...
        self.etcd_data = {
            ...
            '/calico/felix/v1/host/hostname/last_reported_status': json.dumps({
                "uptime": 10,
                "first_update": True}),
            # An agent status key to take notice of.
            '/calico/felix/v1/host/hostname/status': json.dumps({
                "uptime": 10,
                "first_update": True}),
            # A port status key to take notice of.
            '/calico/felix/v1/host/hostname/workload/' +
            'openstack/wlid/endpoint/ep1': '{"status": "up"}',
            # A port status key to ignore.
            '/calico/felix/v1/host/unknown/workload/' +
