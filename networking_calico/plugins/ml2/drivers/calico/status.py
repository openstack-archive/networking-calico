...
class PortStatusWatcher(etcdutils.EtcdWatcher):
    ...
    def __init__(self, calico_driver):
        ...
        # Note that we are _not_ passing a roundtrip suffix to this
        # watcher; because the status subtree is shared by the agent
        # alive updates and port updates, we'll let the less busy
        # of those two (the agent alive watcher), handle writing the
        # round trip heartbeats. This watcher will automatically
        # see them as well.
        ...
        self.register_path(status_path + "/round-trip-check")
...
class AgentStatusWatcher(etcdutils.EtcdWatcher):
    ...
    def __init__(self, calico_driver):
        ...
        # Ignore our own heartbeat events
        self.register_path(status_path + "/round-trip-check")
