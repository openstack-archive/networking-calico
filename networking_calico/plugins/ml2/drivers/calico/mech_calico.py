...
def check_request_etcd_compaction():
    ...
    try:
        ...
        try:
            ...
            # TTL as expected.
            if lease is not None and lease.ttl() <= (
                    cfg.CONF.calico.etcd_compaction_period_mins * 60):
                # It does: just wait for more time to pass then.
                return

            # Lease is missing or has an unreasonably large TTL.  (For example,
            # the former is conceivable if the etcd cluster is restarted after
            # restoring from an incomplete or corrupt backup.)  Start from
            # scratch as though neither of the compaction keys is present.
        ...
        if current_revision <= cfg.CONF.calico.etcd_compaction_min_revisions:
            ...
            # Note that last_compaction_rev must in fact be 0 here.
            write_compaction_keys(0)
            return
