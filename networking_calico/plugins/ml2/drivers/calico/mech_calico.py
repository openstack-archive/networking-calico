            ...
            while self._epoch == expected_epoch:
                ...
                if self.elector.master():
                    ...
                    try:
                        ...
                        self.subnet_syncer.resync_start(admin_context)

                        # Next, resync policies as far as to create any that
                        # are missing from etcd, and to update any that are in
                        # etcd but whose Neutron data differs from etcd.  After
                        # this step, etcd will have correct data for all policy
                        # that an endpoint can be using; that's why we do this
                        # part of the resync before resyncing the endpoints.
                        #
                        # The call also stores a set of policies that are in
                        ...
                        # of those profiles.
                        self.policy_syncer.resync_start(admin_context)

                        # Next, resync endpoints.
                        self.endpoint_syncer.resync_start(admin_context)
                        self.endpoint_syncer.resync_finish()

                        # Now delete the profiles that are no longer wanted.
                        self.policy_syncer.resync_finish()

                        # Delete any other etcd resources that should no longer
                        # be there.
                        self.subnet_syncer.resync_finish()
