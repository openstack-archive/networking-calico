...
calico_opts = [
    ...
    cfg.IntOpt('etcd_compaction_min_revisions', default=1000,
               help="The minimum number of revisions to keep when requesting "
                    "an etcd compaction.  (The maximum number of revisions to "
                    "keep is everything since the previous compaction.)"),
]
...
def check_request_etcd_compaction():
    ...
    try:
        ...
        try:
            etcdv3.get(COMPACTION_TRIGGER_KEY)
            # No exception, so the key still exists - wait for more time to
            # pass.
            return
        except etcdv3.KeyNotFound:
        ...
        try:
            last_compaction, last_check = etcdv3.get(COMPACTION_LAST_KEY)
            last_compaction = int(last_compaction)
            last_check = int(last_check)
            LOG.info("Last compaction %r, last check %r",
                     last_compaction, last_check)
        except etcdv3.KeyNotFound:
        ...
        if current_revision <= cfg.CONF.calico.etcd_compaction_min_revisions:
            LOG.info("Not enough revisions to compact yet")
            write_compaction_keys(last_compaction)
            return
        ...
        if keep_revisions < (current_revision - last_check):
            keep_revisions = current_revision - last_check

        # So that would mean compacting at:
        compact_revision = current_revision - keep_revisions

        if compact_revision <= last_compaction:
            ...
            LOG.info("No compactable history yet (%r <= %r)",
                     compact_revision, last_compaction)
            write_compaction_keys(last_compaction)
            return

        # Request compaction at that revision.
        LOG.info("Request compaction")
