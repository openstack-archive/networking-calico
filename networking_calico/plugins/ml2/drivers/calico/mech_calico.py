...
class CalicoMechanismDriver(mech_agent.SimpleAgentMechanismDriverBase):
    ...
    def maybe_cleanup_port_statuses(self):
        """Retries updates to any dirty port statuses."""
        now = time.time()
    ...
    def _try_to_update_port_status(self, port_status_key):
        ...
        On success, discards it it from self._dirty_port_statuses.
        Otherwise, leaves it in that set for future retries.
    ...
    def _port_bound_update(self, context, port):
        ...
        port = self.add_extra_port_information(self._plugin_context(context),
                                               port)
        profiles = self.get_security_profiles(
