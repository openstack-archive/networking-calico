...
# Interval at which we log opportunistic warnings of queue lengths.
QUEUE_WARN_LOG_INTERVAL = 10
...
# Delay before retrying a failed port status update.
PORT_UPDATE_RETRY_DELAY = 5
...
class CalicoMechanismDriver(mech_agent.SimpleAgentMechanismDriverBase):
    ...
    def __init__(self):
        ...
        # limiting.  Uses the monotonic time epoch.
    ...
    def on_port_status_changed(self, hostname, port_id, status_dict):
        ...
        if self._port_status_cache.get(port_status_key) != calico_status:
            ...
            # the writer threads.  This means that the next write for a
    ...
    def _try_to_update_port_status(self, admin_context, port_status_key):
        ...
        try:
            ...
            eventlet.spawn_after(PORT_UPDATE_RETRY_DELAY,
