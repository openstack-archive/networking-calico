...
class CalicoTransportEtcd(object):
    ...
    @_handling_etcd_exceptions
    def subnet_created(self, subnet_id, cidr, gateway_ip, dns_servers):
        """Write data to etcd to describe a DHCP-enabled subnet."""
        LOG.info("Write subnet %s %s to etcd", subnet_id, cidr)
        data = {'cidr': str(netaddr.IPNetwork(cidr)),
                'gateway_ip': gateway_ip}
        if dns_servers:
            data['dns_servers'] = dns_servers

        self.client.write(datamodel_v1.key_for_subnet(subnet_id),
                          json.dumps(data))
