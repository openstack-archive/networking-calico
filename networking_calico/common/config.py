...
SHARED_OPTS = [
    ...
    cfg.StrOpt('openstack_region',
               help="When in a multi-region OpenStack deployment, a unique "
                    ...
                    "compute) belongs to.  If not set, Calico will use the "
                    "nova.region_name field, so if you already configure "
                    "nova.region_name uniquely across all your regions, you "
                    "do not need to configure this field."),
]
...
def get_region_string():
    ...
    if _cached_region_string is None:
        ...
        region = cfg.CONF.calico.openstack_region
        if region is None:
            # Otherwise use [nova] region_name.
            region = cfg.CONF.nova.region_name

        if region:
            _cached_region_string = "%s%s" % (datamodel_v1.REGION_PREFIX,
                                              region)
        else:
            _cached_region_string = datamodel_v1.NO_REGION
