...
class CalicoDeviceManager(dhcp.DeviceManager):
    ...
    def fill_dhcp_udp_checksums(self, *args, **kwargs):
        ...
        while retries > 0:
            try:
                super(CalicoDeviceManager, self).fill_dhcp_udp_checksums(*args, **kwargs)
            except RuntimeError:
                # fill_dhcp_udp_checksums() can fail transiently if iptables is modified
                # concurrently, especially with an aggressive iptables writer such as Felix running.
                LOG.exception("Failed to insert DHCP checksum rule, may retry...")
                time.sleep(0.1)
                retries -= 1
            else:
            ...
            LOG.error("Consistently failed to insert DHCP checksum rule. Exiting...")
            sys.exit(1)
