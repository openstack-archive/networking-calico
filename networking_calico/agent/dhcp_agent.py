...
class CalicoEtcdWatcher(EtcdWatcher):
    ...
    def on_endpoint_set(self, response, hostname, orchestrator,
        ...
        for ip_version in [4, 6]:
            for id in endpoint.get('ipv%s_subnet_ids' % ip_version):
                LOG.debug("Read subnet %s", id)

                # Read the data for this subnet.
                subnet_key = key_for_subnet(id)
                try:
                    data = safe_decode_json(self.client.read(subnet_key).value,
                                            'subnet')
                    LOG.debug("Subnet data: %s", data)
                except etcd.EtcdKeyNotFound:
                    LOG.debug("No data for subnet %s", id)
                    continue

                # See if we already have this subnet in the NetModel.
                if [s for s in net.subnets if s.cidr == data['cidr']]:
                    ...
                    LOG.debug("old net: %s %s %s",
                              net.id, net.subnets, net.ports)
                    net = dhcp.NetModel(False,
                                        {"id": net.id,
                                         "subnets": net.subnets + [subnet],
                                         "ports": net.ports,
                                         "tenant_id": "calico"})
                    LOG.debug("new net: %s %s %s",
                              net.id, net.subnets, net.ports)

                    # Next line - i.e. just discarding the existing cache - is
                    # to work around Neutron bug that the DHCP port is not
                    # entered into the cache's port_lookup dict.
            ...
            for addrm, id in zip(endpoint['ipv%s_nets' % ip_version],
