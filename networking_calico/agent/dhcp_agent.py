...
class CalicoEtcdWatcher(etcdutils.EtcdWatcher):
    ...
    def on_endpoint_set(self, response, name):
        ...
        fixed_ips = []
        dns_assignments = []
        fqdn = endpoint.get('fqdn')  # FIXME, annotation?
        ...
        port = {'id': endpoint_id,
                ...
                'extra_dhcp_opts': []}  # FIXME, annotation?
        if fqdn:
    ...
    def _ensure_net_and_subnets(self, port):
        ...
        for subnet_id in needed_subnet_ids:
            try:
                ...
                new_subnets[subnet_id] = self._get_subnet(subnet_id)
            except KeyError:
    ...
    def _get_subnet(self, subnet_id):
        # Get data for subnet SUBNET_ID and translate it to the dict form
        # expected for insertion into a Neutron NetModel.
        LOG.debug("Get subnet %s", subnet_id)

        data = self.subnet_watcher.subnets_by_id[subnet_id]
