...
#
# So, here we check for that mistake, and correct it.  Note that correcting this
# mistake equally means that we are breaking requests somehow - but that doesn't
# matter because the Calico DHCP agent doesn't actually execute any requests
# code.  (To be clear: the Calico DHCP agent imports a slew of Neutron utility
# modules, which in turn import other Neutron and Oslo and even Keystone modules,
# and so on, and some of those import requests.  But the Calico DHCP agent
# actually executes only a small subset of all that code.)
