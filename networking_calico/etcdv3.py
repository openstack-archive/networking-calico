...
def put(key, value, mod_revision=None, ttl=None, existing_value=None):
    ...
    - mod_revision (string): If specified, indicates that the write should only
      ...
      mod_revision=0 indicates that the must not currently exist.

    - ttl (int, in seconds): If specified, indicates that the write should have
      that TTL.
...
def delete(key, existing_value=None):
...
def get_prefix(prefix):
    ...
    for result in results:
        value, item = result
        tuple = (item['key'], value, item['mod_revision'])
        tuples.append(tuple)
    return tuples
