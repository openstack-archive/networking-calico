...
class ForceClosingWatcher(watch.Watcher):
    def stop(self):
        try:
            # Workaround for (at least) Python 2.7.12.  Shut down the socket to
            # force close both legs of the connection.  Without this, the socket
            # sticks waiting for some data to read.
            #
            # We make our own socket from the file handle so that we disturb the
            # real socket as little as possible.  From the real socket's point of
            # view, the socket will appear to fail.
